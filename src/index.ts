...
import { TEmbeddingVector, TIndexItem, TScoredIndexItem } from "./types.js";
...
const buildIndex = async (inputPath: string): Promise<TIndexItem[]> => {
  ...
  const isAudio = [".mp3", ".wav", ".m4a", ".ogg", ".webm", ".flac"].some(ext => lower.endsWith(ext));
  const text = isAudio ? await transcriptAudio(inputPath) : await extractPdfText(inputPath);
 ...
 */
const retrieve = async (index: TIndexItem[], query: string, k: number = 5): Promise<TScoredIndexItem[]> => {
 ...
 */
const answer = async (contextChunks: Array<{ text: string }>, question: string): Promise<string> => {
