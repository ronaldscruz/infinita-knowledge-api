...
export type Vector = { id: string; values: number[]; metadata: Record<string, any> };

export async function upsertVectors(vectors: Vector[], batchSize = 200): Promise<void> {
...
export async function queryIndex(vector: number[], topK: number, includeMetadata = true): Promise<any> {
  // @ts-ignore
