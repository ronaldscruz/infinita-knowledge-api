...
export async function ingestFromSources(uploadedPdfPaths: string[], youtubeUrls: string[], rawTexts: string[]): Promise<{ upserted: number }>{
  ...
  for (const t of rawTexts) collected.push({ source: "raw", kind: "text", text: t });
  ...
  for (const item of collected) {
    ...
    embs.forEach((values, idx) => {
      ...
      vectors.push({ id, values, metadata: { source: item.source, kind: item.kind, chunk_index: idx, text: chunks[idx] } });
    });
  }
