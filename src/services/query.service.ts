...
import { buildAnswerPrompt, buildAnalysisPrompt, buildOverviewPrompt, buildQuizPrompt, buildSummaryPrompt } from "../prompts/index.js";
...
export async function generate(mode: QueryMode, context: string, question: string): Promise<string> {
  const client = getOpenAiClient();
  const pair = (
    mode === "answer" ? buildAnswerPrompt(context, question) :
    mode === "summary" ? buildSummaryPrompt(context) :
    mode === "overview" ? buildOverviewPrompt(context) :
    mode === "analysis" ? buildAnalysisPrompt(context) :
